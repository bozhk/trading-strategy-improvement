...
impl StreamManager {
    pub fn new() -> Self {
        Self { tasks: Vec::new(), symbols: Vec::new(), demo: false, generation: Arc::new(AtomicU64::new(0)) }
    }
    ...
    pub async fn set_symbols(&mut self, symbols: Vec<String>, demo: bool) {
        ...
        if demo {
            ...
            self.tasks.push(tokio::spawn(demo_feed(symbols, generation, gen_ref)));
            STATE.lock().log("DEMO", "Deterministic demo feed active; results reset", "SYSTEM", 0.0);
        } else {
            for chunk in symbols.chunks(SETTINGS.ws_chunk_size) {
                let gen_ref = self.generation.clone();
                self.tasks.push(tokio::spawn(live_feed(chunk.to_vec(), generation, gen_ref)));
            }
        }
    }
}
        ...
        match tokio_tungstenite::connect_async(&SETTINGS.ws_url).await {
            Ok((mut ws, _)) => {
                let mut args: Vec<String> =
                    symbols.iter().map(|s| format!("orderbook.50.{s}")).collect();
                        ...
                        match ws.next().await {
                            Some(Ok(Message::Text(raw))) => {
                                if let Ok(msg) = serde_json::from_str::<Value>(&raw) {
                                    ...
                                    STATE.lock().log("WARN", "Malformed stream payload", "SYSTEM", 10.0);
                                }
                            }
...
fn handle_message(msg: &Value) {
    ...
    if topic.starts_with("orderbook") && !data.is_null() {
        ...
        let mut state = STATE.lock();
        let book = state.books.entry(symbol.to_string()).or_insert_with(OrderBook::default);
        ...
        if let Some(rows) = data.as_array() {
            ...
            for t in rows {
                ...
                state.push_tick(&symbol, TradeTick {
                    timestamp: ts / 1000.0,
                    is_buy: side == "Buy",
                    price,
                    size,
        ...
        ticks += 1;
        {
            ...
            for (idx, symbol) in symbols.iter().enumerate() {
                ...
                for level in 1..=50_i32 {
                    let mut size = (8.0 + seed.random::<f64>() * 14.0) * (1.0 + bias.max(0.0) * 1.7);
                ...
                let book = state.books.entry(symbol.clone()).or_insert_with(OrderBook::default);
                book.apply("snapshot", &bids, &asks, ticks as i64);
                let is_buy = seed.random::<f64>() < 0.5 + bias * 0.28;
                let size = seed.random_range(2.0..45.0);
                state.push_tick(symbol, TradeTick { timestamp: now_ts(), is_buy, price, size });
            }
        }
